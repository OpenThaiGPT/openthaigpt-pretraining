...
from openthaigpt_pretraining_data.decontamination.utils import generate_minhash_signature
...
def decontaminate(dataset_groups, decontaminate_args):
    ...
    for dataset_key in tqdm(dataset_groups.keys()):
        ...
        pretrain_dataset_minhash_result = pretrain_dataset_minhash.map(
            ...
            lambda x: len(x["__neighbors__"]) > 0 and not np.array_equal(x['hashvalues'], empty_hashvalues),
            desc=f"Filtering...",
            num_proc=decontaminate_args.num_process
        )
        print(pretrain_dataset_minhash_result, 'pretrain_dataset_minhash_result')

        for doc in tqdm(pretrain_dataset_minhash_result, desc="Calculation Jaccard Distance..."):
            neighbors = set(doc['__neighbors__'])
            for neighbor in neighbors:
                reference = data[int(neighbor)]
                reference_signature = signature[int(neighbor)]
                score = LeanMinHash(seed=MINHASH_SEED, hashvalues=doc['hashvalues']).jaccard(reference_signature)
